...
```{r}
birds_type<- group_summarize(birds, type)
...
```{r}
ggplot(birds_type, aes(egg_mass_median, male_mass_median)) +
  geom_point() +
  scale_x_log10() +
  scale_y_log10()
...
```{r}
birds_new <- filter(birds_type, n > 5)
...
ggplot(birds_type, aes(egg_mass_median, male_mass_median)) +
  ...
  geom_text(aes(label = type),color = "red", data = birds_new)
