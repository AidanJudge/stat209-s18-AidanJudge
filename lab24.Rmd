...
```{r}
crimes_night_ca <- filter(crimes_time_ca, night_flag == 1)
crimes_night_ca$robbery_rate <- (crimes_night_ca$robbery_sum/(crimes_night_ca$robbery_sum + crimes_night_ca$theft_sum))

ggplot(crimes_night_ca, aes(median_income, robbery_rate)) +
  geom_point(aes(color = factor(night_flag))) +
  geom_smooth(aes(color = factor(night_flag)), method = "lm") +
  scale_color_viridis(discrete = TRUE) +
  ggtitle("Robberies as a Ratio of Total Thefts and Robberies at Night") +
  ylab("% of Robberies to Thefts and Robberies") +
  xlab("Median Income")
```

##THESIS

For neighborhoods with higher median incomes, the ratio of robberies to the total number of robberies and thefts decreases sharply for crimes occurring at night.