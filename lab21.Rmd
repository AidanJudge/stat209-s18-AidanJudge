...
**Answer**: The bar plot works because it shows somewhat of a bell-curve distribution of the numeric ratings.

Produce a confidence interval for the mean of your trait.

```{r}
model1 <- lm_basic(emotional_stability ~ 1, data=pf)
...
model2 <- lm_basic(emotional_stability ~ 1, data=Hungary)
...
**Answer**: This confidence interval has a larger range which makes sense because there are only 55 observations as opposed to 49,060 observations which realistically allows for a smaller margin of error.
...
```{r}
model3 <- lm_basic(emotional_stability ~ 1 + gender, data=pf)
reg_table(model3, level=.95)
```

**Answer**: Between -.02332 and .006.
...
**Answer**:  For emotional stability, there is not a statistically significant difference between men and women since 0 falls within the 95% confidence interval.  This might challenge the traditional gender stereotypes that women are less emotionally stable than men.
...
```{r}
adults <- filter(pf, age >= 30, age <= 49)
adults$fourties <- adults$age >=40
```
...
```{r}
model4 <- lm_basic(emotional_stability ~ 1 + fourties, data=adults)
...
**Answer**: There is not a statistically significant difference in emotional stability between people in their thirties and fourties because 0 falls within the 95% confidence interval.
